...
function Item({
  ...
  const classes = `group flex min-h-10 items-center justify-between px-2.5 py-2 rounded-xl cursor-pointer transition-colors duration-200 select-none ${
    ativo
    ...
    <>
      <div className="flex items-center gap-2.5 min-w-0">
        <item.icone
          ...
          strokeWidth={1.5}
        />
        <span className="text-[13px] truncate">{item.titulo}</span>
      </div>
  ...
  return (
    <div className="flex flex-col w-full">
      {temFilhos ? (
        <button type="button" aria-expanded={aberto} className={`${classes} w-full text-left`} style={estilo} onClick={() => setAberto(!aberto)}>
          {conteudo}
        </button>
      ) : item.acao ? (
        ...
        <Link href={item.href ?? '#'} aria-current={ativo ? 'page' : undefined} className={classes} style={estilo}>
          {conteudo}
        </Link>
      )}
...
export function SidebarNav({
  ...
  return (
    <div className={`admin-glass flex flex-col w-[260px] h-full border-r border-border/50 p-4 font-sans ${className}`}>
      ...
      <div className="flex-1 overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] flex flex-col gap-4 mt-2">
        {grupos.map((grupo, i) => (
          <div key={i} className="flex flex-col gap-0.5">
            {grupo.titulo && (
              <span className="px-2.5 mb-1 text-[11px] font-semibold text-muted-foreground">
                {grupo.titulo}
              </span>
            )}
